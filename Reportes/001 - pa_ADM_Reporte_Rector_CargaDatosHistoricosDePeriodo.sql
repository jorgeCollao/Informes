 ...
 ************************************************************/
CREATE  PROCEDURE pa_ADM_Reporte_Rector_CargaDatosHistoricosDePeriodo(@DIAUNO DATETIME ,@AÑO INT ,@CODCARR VARCHAR(30) = NULL)
AS
BEGIN
	IF @AÑO < 2016
	BEGIN
	...
	SET @FECHA_BISIESTO = (
	    ...
	    SELECT DISTINCT(
	          ...
	          ,(
	               ...
	               FROM   (
	                          ...
	                          WHERE  A1.ANO = @AÑO
	                                 AND A1.PERIODO = 1
	                            --     AND A1.ESTACAD IN ('VIGENTE' ,'SUSPENDIDO')
	                                 AND A1.CODCARPR = A.CODCARPR
	                                 AND CASE 
	                                          WHEN DATEDIFF(DAY ,@DIAUNO ,M.FECHA_REGISTRO) <= 0 THEN 1
	                                          ELSE DATEDIFF(DAY ,@DIAUNO ,M.FECHA_REGISTRO)+1
	    ...
	    WHERE  A.ANO = @AÑO
	           ...
	           --AND
				--m.CODCLI  NOT IN (SELECT RS.CODCLI
				--						FROM   RA_SITU AS RS
				--								INNER JOIN RA_TIPOSITU AS RT
				--									ON  RS.TIPOSITU = RT.CODIGO
				--										AND rs.EMISION <= '30-04-' + LTRIM(CONVERT(VARCHAR ,@AÑO))
				--						WHERE  RT.ESTACAD IN ('ELIMINADO' ,'TITULADO' ,'RETRACTADO' ,'EGRESADO'))               
	    ORDER BY
	           A.CODCARPR
